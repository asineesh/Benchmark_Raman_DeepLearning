...
def main():
    ...
    os.makedirs("results/hyperparameter_tuning/Bacteria_ID/eight/", exist_ok=True) #where the logs go
    os.makedirs("results/hyperparameter_tuning/Bacteria_ID/models/", exist_ok=True) #where the pretrained models go
    os.makedirs("results/hyperparameter_tuning/trained_models/", exist_ok=True) #where the finetuned models go
    
    filename = "results/hyperparameter_tuning/Bacteria_ID/eight/results_RamanNet.txt"
    print(device)
            ...
            for epoch in range(epochs):
                ...
                if acc>best_val_acc:
                    ...
                    if best_val_acc>all_best_val_acc:
                        ...
                        #Saving the current model
                        best_final_model_name = f"results/hyperparameter_tuning/trained_models/Bacteria_eight_RamanNet_{epoch}_{round(acc,2)}_.pt"
