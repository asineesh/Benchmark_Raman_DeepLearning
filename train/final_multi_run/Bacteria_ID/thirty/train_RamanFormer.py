...
def main():
    ...
    parser.add_argument("--learning_rate", type=float, help="The learning rate",default=0.0001)
    ...
    os.makedirs("results/final_multi_run/Bacteria_ID/thirty", exist_ok=True)
    os.makedirs("results/final_multi_run/trained_models/", exist_ok=True)

    filename = "results/final_multi_run/Bacteria_ID/thirty/results_RamanFormer.txt"
    ...
    for run in range(args.runs):
        ...
        train_set = Bacteria_Dataset("datasets/Bacteria_ID/X_reference.npy","datasets/Bacteria_ID/y_reference.npy",30)
        ...
        #Pretraining
        model = RamanFormer(patch_size=125,sp_size=1000,num_classes=30,p=0.1).to(device)
        ...
        for epoch in range(epochs):
            ...
            if acc>best_val_acc:
                ...
                best_final_model_name = f"results/final_multi_run/trained_models/Bacteria_ID_thirty_RamanFormer_pre_{run}_{epoch}_{round(acc,2)}_.pt"
        ...
        pretrained_model = RamanFormer(patch_size=125,sp_size=1000,num_classes=30,p=0.1).to(device)
        ...
        for epoch in range(epochs):
            ...
            if acc>best_val_acc:
                ...
                #Saving the current model
                best_final_model_name = f"results/final_multi_run/trained_models/Bacteria_ID_thirty_RamanFormer_fine_{run}_{epoch}_{round(acc,2)}_.pt"
